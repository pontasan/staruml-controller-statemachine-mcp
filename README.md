# StarUML Controller MCP Server — State Machine Diagram

An MCP (Model Context Protocol) server specialized for **State Machine Diagrams**. Enables AI assistants like Claude to programmatically create and manage state machine diagrams — states (with regions), pseudostates, final states, and transitions in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="EC Order Processing State Machine — composite states, fork/join, choice/junction, all 10 pseudostate kinds" width="800">
</p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-statemachine-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-statemachine-mcp.git
cd staruml-controller-statemachine-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all State Machine Diagram tools become available via MCP
...
{
  "mcpServers": {
    "staruml-statemachine": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-statemachine-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-statemachine node /absolute/path/to/staruml-controller-statemachine-mcp/dist/index.js
```
...
The core of this server — full CRUD for every State Machine Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `statemachine_list_diagrams`, `statemachine_create_diagram`, `statemachine_get_diagram`, `statemachine_update_diagram`, `statemachine_delete_diagram` |
| **States** | `statemachine_list_states`, `statemachine_create_state`, `statemachine_get_state`, `statemachine_update_state`, `statemachine_delete_state` |
| — Regions | `statemachine_list_state_regions`, `statemachine_create_state_region` |
| **Pseudostates** | `statemachine_list_pseudostates`, `statemachine_create_pseudostate`, `statemachine_get_pseudostate`, `statemachine_update_pseudostate`, `statemachine_delete_pseudostate` |
| **Final States** | `statemachine_list_final_states`, `statemachine_create_final_state`, `statemachine_get_final_state`, `statemachine_update_final_state`, `statemachine_delete_final_state` |
| **Transitions** | `statemachine_list_transitions`, `statemachine_create_transition`, `statemachine_get_transition`, `statemachine_update_transition`, `statemachine_delete_transition` |
